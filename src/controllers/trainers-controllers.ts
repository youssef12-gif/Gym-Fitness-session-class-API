...
const findClassSession = async(ID: Types.ObjectId)=> {
    try{
...
export const modifyClassSession = async(req:Request , res:Response)=> {
    const {_id} = req.body;
    if(!_id){
    ...
    try{
        const updatedClassSession = await ClassSession.updateOne(
            {_id: _id} , 
            {$set: {title: title , 
                trainer: fullName,
                timeslot: timeslot,
