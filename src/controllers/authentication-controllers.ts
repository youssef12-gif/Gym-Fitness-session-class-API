...
const createToken = (_id:mongoose.Types.ObjectId , fullName:string , role:string) =>{
  const token = Jwt.sign({_id , fullName  , role} , process.env.JWT_SECRET as string , {expiresIn: '1h'});
  return token;
}
